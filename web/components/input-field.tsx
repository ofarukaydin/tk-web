...
const InputField = (props: PropTypes): JSX.Element => {
  ...
  if (props.type === 'radio') {
    return (
      <label tw="inline-flex items-center ">
        <input type="radio" tw="form-radio text-green-600" {...field} {...props} id={props.name} />
  ...
  return (
    <div>
      ...
      <input tw="form-input mt-1 block w-full bg-gray-100" {...field} {...props} id={props.name} />
      {error && <div>{error}</div>}
    </div>
  );
};
